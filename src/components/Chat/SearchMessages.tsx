...
export function SearchMessages() {
	...
	useEffect(() => {
		...
		searchTerm
			? setMessageFilter(
					messages?.filter(
						msg => msg.type === 'text' && msg.message.includes(searchTerm)
					)
			  )
			: setMessageFilter([])
	}, [searchTerm])
	return (
		<div className='h-full w-full relative overflow-auto custom-scrollbar grow'>
			<span className='p-2 absolute px-5'>
				<IoClose
					className='h-8 w-8 text-gray-400 hover:text-gray-800 cursor-pointer'
					onClick={() => changeIsSearchMessage(false)}
				/>
			</span>
				...
				{searchTerm &&
					...
					messageFilter.map(message => (
						<div className='flex flex-col' key={message.id}>
							...
							<span className='text-teal-600 text-xl'>{message.message}</span>
						</div>
					))}
			</div>
		</div>
